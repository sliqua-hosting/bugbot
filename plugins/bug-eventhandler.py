...
class RabbitMQ_Worker(object):
    def __init__(self, callback):
        self.queue = socket.gethostname() + 'bugbot'
        self.exchange = 'magfest.techops.notifications'
        self.exchange_type = 'fanout'
        self.routing_key = 'json'
    ...
    def connect(self):
        ...
        return pika.SelectConnection(pika.ConnectionParameters('localhost'),self.on_connection_open, stop_ioloop_on_close=False)
...
def process_amqp_message(ch, method, properties, body):
    #LOGGER.info('bugbot Received message # %s from %s: %s', basic_deliver.delivery_tag, properties.app_id, body)
    output.append([ 'C04TMV1LM', message ])
    return
...
def setup(self):
    LOGGER.info("AMQP Event Handler Plugin loaded!")
    ...
    amqp_worker = RabbitMQ_Worker(bugbot_sendjson)
    try:
        amqp_worker.run()
